...
class Fields_Map_Table extends WP_List_Table {
    ...
    function column_default($item, $column_name){
        switch($column_name){
			...
			case 'custom_label':
				return '<input type="text" name="wp_meta_label[]" data-stored="'.$item['wp_custom_label'].'" value="'.$item['wp_custom_label'].'" style="width: 160px;" maxlength="50" class ="OEPL_custom_label" /><a class="OEPL_small_button OEPL_save_custom_label" data-pid='.$item['pid'].' title="Save Custom Label"><i class="fa fa-check-square fa-2x"></i></a>';            
            case 'is_show':
				$isshow = '';
				if($item['is_show'] == 'Y')
            ...
            case 'display_order':
				return '<input type="text" name="display_order[]" data-stored="'.$item['display_order'].'" value="'.$item['display_order'].'" style="width: 40px;" maxlength="2" class ="OEPL_custom_order OEPLIntInput" /><a class="OEPL_small_button OEPL_save_custom_order" data-pid='.$item['pid'].' title="Save Custom Label"><i class="fa fa-check-square fa-2x"></i></a>';  
              
                //return '<input type="text" name="display_order[]" value='.$item['display_order'].' style="width:35px;" maxlength="3" class ="display_order OEPLIntInput" /><input type="hidden" name="display_order_ID[]" value='.$item['pid'].' />';
            case 'required':
				$isshow = '';
				if($item['required'] == 'Y')
					$isshow = "checked='checked'";
			...
			case 'hidden_field':
				if ($item['custom_field'] == "Y"){
					...
					if($item['hidden'] == 'Y')
						$isshow = "checked='checked'";
					return '<input type="checkbox" data-action="OEPL_Change_Hidden_Status" data-pid='.$item['pid'].' class="OEPL_is_hidden OEPL_grid_status" value="1" '.$isshow.'>';	
				}
            default:
