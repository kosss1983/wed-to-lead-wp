		...
		if($this->SugerSessID)
		{
			...
			for($i=0; $i<$fCnt; $i++)
			{
				if( isset($_POST[$ContRS[$i]['wp_meta_key']]) )
				{
					...
					// mapping source and device to SuiteCRM
					switch ($ContRS[$i]['field_name']) {
							case "campaign_code_c":
									if(!empty($_SESSION['cCode'])){
											$name_value_list[] = array("name" => $ContRS[$i]['field_name'], "value" => $_SESSION['cCode']);
									}
									break;
							case "source_c":
									if(!empty($_SESSION['source'])){
											$name_value_list[] = array("name" => $ContRS[$i]['field_name'], "value" => $_SESSION['source']);
									}
									break;
							case "device_c":
									if(!empty($_SESSION['device'])){
											$name_value_list[] = array("name" => $ContRS[$i]['field_name'], "value" => $_SESSION['device']);
									}
									break;
							case "website":
									if(!empty($_SESSION['url'])){
											$name_value_list[] = array("name" => $ContRS[$i]['field_name'], "value" => $_SESSION['url']);
									}
									break;
							default:
