...
add_action('wp_ajax_OEPL_save_custom_label','OEPL_save_custom_label_callback');
function OEPL_save_custom_label_callback(){
	global $wpdb;
	$upd = $wpdb->update( OEPL_TBL_MAP_FIELDS, array('wp_custom_label' => $_POST['label']), array('pid' => (int)$_POST['pid']));
	$response = array();
	if($upd != FALSE && $upd>0){
		$response['STATUS'] = 'Y';
		$response['Message'] = 'Custom Label changed succesfully';
	} else {
		$response['STATUS'] = 'N';
		$response['Message'] = 'Error occured while saving custom label. Please try again';
	}
	echo json_encode($response);
	exit;
}

add_action('wp_ajax_OEPL_save_custom_order','OEPL_save_custom_order_callback');
function OEPL_save_custom_order_callback(){
	global $wpdb;
	
	echo "<pre>"; print_r($_POST); 
	$upd = $wpdb->update( OEPL_TBL_MAP_FIELDS, array('display_order' => (int)$_POST['label']), array('pid' => (int)$_POST['pid']));
	$response = array();
	if($upd != FALSE && $upd>0){
		$response['STATUS'] = 'Y';
		$response['Message'] = 'Display order changed succesfully';
	} else {
		$response['STATUS'] = 'N';
		$response['Message'] = 'Error occured while saving display order value. Please try again';
	}
	echo json_encode($response);
	exit;
}

add_action('wp_ajax_OEPL_Grid_Ajax_Action','OEPL_Grid_Ajax_Action_callback');
function OEPL_Grid_Ajax_Action_callback(){
	global $wpdb;
	$action = $_POST['OEPL_Action'];
	//$updData = array('pid' => $_POST['pid']);
	
	$upData = array();
	$flag = FALSE;
	if($action == 'OEPL_Change_Status'){
		$flag = TRUE;
		$sql = "SELECT is_show FROM ".OEPL_TBL_MAP_FIELDS." WHERE pid = ".(int)$_POST['pid'];
		...
		$flag = TRUE;
		$sql = "SELECT hidden FROM ".OEPL_TBL_MAP_FIELDS." WHERE pid = ".(int)$_POST['pid'];
		$status = $wpdb->get_row($sql, ARRAY_A);
		...
		$flag = TRUE;
		$sql = "SELECT required FROM ".OEPL_TBL_MAP_FIELDS." WHERE pid = ".(int)$_POST['pid'];
		$status = $wpdb->get_row($sql, ARRAY_A);
		if(isset($status['required']) && !empty($status)){
			if($status['required'] == 'Y'){
				$updData['required'] = 'N';
			} else {
				$updData['required'] = 'Y';
			}
	...
	if($flag && isset($updData) && !empty($updData)) {
		...
		if($upd != FALSE && $upd>0){
			$respone['message'] = 'Field Updated Successfully';
		} else {
			$respone['message'] = 'Error While updating field. Please try again';
		}
	} else {
		$respone['message'] = 'Error While updating field. Please try again';
	}
...
/*
 * register device and source values in session
 */
...
function register_my_session()
{
	...
	if( session_id() ){
		...
		if ( !empty($_REQUEST['cCode']) ) {
		  $_SESSION['cCode'] = $_REQUEST['cCode'];
		}

		if ( !empty($_REQUEST['s']) ) {
		  $_SESSION['source'] = $_REQUEST['s'];
		}

		if ( wp_is_mobile() ) {
		  $_SESSION['device'] = 'mobile';
		} else {
		  $_SESSION['device'] = 'no-mobile';
		}         

		$_SESSION['url'] = get_site_url();
	}
}
